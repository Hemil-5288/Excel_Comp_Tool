...
def compare_excel_with_gain_summary_inline(
    ...
    for sheet_name, cfg in sheets_config.items():
        try:
            ...
            if sheet_name == "Gain Summary":
                ...
                for orig_idx, orig_row in df_orig.iterrows():
                    ...
                    for i, col in enumerate(common_cols, start=1):
                        ...
                        c_diff = main_ws.cell(row=out_row, column=(i - 1) * 3 + 3, value=diff_val)
            ...
            # 8938 
            elif sheet_name == "8938" and ("Account Number" in df_orig.columns) and ("Account Number" in df_web.columns):
                headers = []
                for col in common_cols:
                    headers.extend([f"{col} (Original)", f"{col} (Website)", f"{col} (Diff)"])
                for ci, h in enumerate(headers, start=1):
            ...
            # FBAR 
            elif sheet_name == "FBAR" and ("Account Number" in df_orig.columns) and ("Account Number" in df_web.columns):
                headers = []
                for col in common_cols:
                    headers.extend([f"{col} (Original)", f"{col} (Website)", f"{col} (Diff)"])
                for ci, h in enumerate(headers, start=1):
            ...
            # Common rows sheet for every sheet
